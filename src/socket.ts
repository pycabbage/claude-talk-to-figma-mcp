...
function handleConnection(ws: ServerWebSocket<any>) {
  ...
  try {
    ...
    logger.error(
      `Failed to send welcome message to client ${clientId}:`,
      error,
    );
    stats.errors++;
  }
    ...
    channels.forEach((clients, channelName) => {
      if (clients.has(ws)) {
        ...
        try {
          ...
          logger.error(
            `Error notifying channel ${channelName} about client disconnect:`,
            error,
          );
          stats.errors++;
        }
...
const server = Bun.serve({
  ...
  fetch(req: Request, server) {
    ...
    return new Response(
      "Claude to Figma WebSocket server running. Try connecting with a WebSocket client.",
      {
        headers: {
  ...
  websocket: {
      ...
      try {
        ...
        if (data.type === "join") {
          ...
          if (!channelName || typeof channelName !== "string") {
            logger.warn(
              `Client ${clientId} attempted to join without a valid channel name`,
            );
            ws.send(
              JSON.stringify({
                type: "error",
          ...
          try {
            ...
            logger.debug(
              `Connection confirmation sent to client ${clientId} for channel ${channelName}`,
            );
          } catch (error) {
            logger.error(
              `Failed to send join confirmation to client ${clientId}:`,
              error,
            );
            stats.errors++;
          }
          ...
          try {
            ...
            if (notificationCount > 0) {
              logger.debug(
                `Notified ${notificationCount} other clients in channel ${channelName}`,
              );
            }
          } catch (error) {
        ...
        if (data.type === "message") {
          ...
          if (!channelName || typeof channelName !== "string") {
            logger.warn(
              `Client ${clientId} sent message without a valid channel name`,
            );
            ws.send(
              JSON.stringify({
                type: "error",
          ...
          if (!channelClients || !channelClients.has(ws)) {
            logger.warn(
              `Client ${clientId} attempted to send to channel ${channelName} without joining first`,
            );
            ws.send(
              JSON.stringify({
                type: "error",
          ...
          try {
            ...
            channelClients.forEach((client) => {
              if (client.readyState === WebSocket.OPEN) {
                logger.debug(
                  `Broadcasting message to client in channel ${channelName}`,
                );
                client.send(
                  JSON.stringify({
                    type: "broadcast",
            ...
            logger.info(
              `Broadcasted message to ${broadcastCount} clients in channel ${channelName}`,
            );
          } catch (error) {
            logger.error(
              `Error broadcasting message to channel ${channelName}:`,
              error,
            );
            stats.errors++;
          }
        ...
        if (data.type === "progress_update") {
          ...
          if (!channelName || typeof channelName !== "string") {
            logger.warn(
              `Client ${clientId} sent progress update without a valid channel name`,
            );
            return;
          }
          ...
          if (!channelClients) {
            logger.warn(
              `Progress update for non-existent channel: ${channelName}`,
            );
            return;
          }
    ...
    close(ws: ServerWebSocket<any>, code: number, reason: string) {
      const clientId = ws.data?.clientId || "unknown";
      logger.info(
        `WebSocket closed for client ${clientId}: Code ${code}, Reason: ${reason || "No reason provided"}`,
      );
      ...
      channels.forEach((clients, channelName) => {
        if (clients.delete(ws)) {
          logger.debug(
            `Removed client ${clientId} from channel ${channelName} due to connection close`,
          );
        }
      });
...
logger.info(
  `Status endpoint available at http://localhost:${server.port}/status`,
);
