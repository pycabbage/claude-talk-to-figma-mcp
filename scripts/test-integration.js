  ...
  try {
    if (!fs.existsSync(configPath)) {
      ...
      const shouldConfigure = await askQuestion(
        "Do you want to configure Claude Desktop now? (y/n)",
      );

      if (shouldConfigure.toLowerCase() === "y") {
        log.info("Running configuration script...");
        execSync("bun run configure-claude", {
          stdio: "inherit",
          cwd: rootDir,
        });
...
async function startWebSocketServer() {
  ...
  if (portInUse) {
    log.warning(
      "Port 3055 is already in use. Possibly the WebSocket server is already running.",
    );
    const shouldContinue = await askQuestion(
      "Do you want to continue with tests? (y/n)",
    );
  ...
  wsServer.stderr.on("data", (data) => {
    console.error(
      `${colors.red}[WebSocket Error]${colors.reset} ${data.toString().trim()}`,
    );
  });
...
async function checkFigmaPlugin() {
  ...
  try {
    ...
    log.info(
      "The plugin code is located in the src/claude_mcp_plugin directory",
    );

    // Ask if the user has already installed the plugin
    const isPluginInstalled = await askQuestion(
      "Have you installed the Claude MCP Plugin as a development plugin in Figma? (y/n)",
    );
    if (isPluginInstalled.toLowerCase() !== "y") {
      ...
      log.info(
        "4. Navigate to and select the folder `src/claude_mcp_plugin` from this repository",
      );
      return false;
    } else {
...
async function runIntegrationTests() {
  ...
  log.info(
    "3. Open Figma and run the Claude MCP Plugin from your Development plugins",
  );
  ...
  if (wsServer) {
    const keepServerRunning = await askQuestion(
      "Do you want to keep the WebSocket server running? (y/n)",
    );
